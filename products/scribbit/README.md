...
| `scribbit-playground` | `@bsh/scribbit-playground` | app | `apps/playground` | The Signet Playground (ADR-0009): a guided five-step first inscription on signet in under five minutes (throwaway browser key or wallet-kit, PoW faucet, exact quote, commit + reveal, certificate + quiz); `?demo=1` fully offline |
| `scribbit-signet-faucet` | `@bsh/scribbit-signet-faucet` | service | `services/signet-faucet` | Signet-only faucet: SHA-256 proof-of-work challenges, per-address/per-IP buckets, daily budget, bitcoind `sendtoaddress` port (off by default). Provides `contracts/openapi/scribbit-signet-faucet.yaml` |
| `scribbit-playground-kit` | `@bsh/scribbit-playground-kit` | library | `packages/playground-kit` | The playground's step explanations, glossary and quiz (shared with the MCP server) and the faucet's PoW rule |
